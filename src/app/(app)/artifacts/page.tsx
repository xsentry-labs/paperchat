      ...
      <div className="flex items-center justify-between border-b border-border/50 px-6 py-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-medium text-foreground">
            {openFolder ? FOLDERS.find((f) => f.type === openFolder)?.label ?? "Folder" : "Artifacts"}
          </h1>
          <span className="text-[10px] text-muted-foreground/40">
            {openFolder ? `${folderDocs.length} file${folderDocs.length !== 1 ? "s" : ""}` : `${documents.length} file${documents.length !== 1 ? "s" : ""}`}
          </span>
        </div>
      </div>

      {error && (
        <div className="mx-6 mt-3 rounded-md bg-destructive/5 border border-destructive/10 px-3 py-2 text-[11px] text-destructive flex items-center gap-2">
        ...
        {!openFolder ? (
          <div className="space-y-8">
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`flex flex-col items-center gap-3 rounded-xl border border-dashed py-8 px-6 transition-colors ${
                dragging
            ...
            >
              ...
              <p className="text-[10px] text-muted-foreground/30">PDF, TXT, MD, DOCX up to 10MB</p>
            </div>
          ...
          /* File list — no expand, just rows with summary + delete */
          <div>
            {folderDocs.length === 0 ? (
              ...
              <div className="rounded-xl border border-border/40 overflow-x-auto">
                {/* Column headers */}
                <div className="grid grid-cols-[1fr_80px_100px_80px_32px] min-w-[600px] gap-2 px-4 py-2 text-[10px] text-muted-foreground/40 uppercase tracking-wider border-b border-border/30 bg-card/30">
                  <span>Name</span>
                  <span>Size</span>
                  <span>Date</span>
                  <span>Status</span>
                  ...
                  return (
                    <div key={doc.id} className="border-b border-border/20 last:border-0">
                      <div
                        ...
                        className={`group grid grid-cols-[1fr_80px_100px_80px_32px] min-w-[600px] gap-2 items-center px-4 py-2.5 cursor-pointer transition-all duration-100 ${
                      ...
                      {isExpanded && (
                        <div className="px-4 pb-3 pt-1 bg-hover/50 animate-fade-in">
                          <div className="ml-5">
                            {doc.summary ? (
                              <div>
